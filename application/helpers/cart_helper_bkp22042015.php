...
// 				if($session_prefix=='')
// 				{
// 					$sql = $CI->db->insert_string("customer_cartwish",$data)." ON DUPLICATE KEY UPDATE product_qty=".(int)$cartArr[$customer_id][$prod_price_id]['qty'].", 
...
// 					$data["manufacturer_id"] = MANUFACTURER_ID;
// 					$sql = $CI->db->insert_string("customer_cartwish",$data)." ON DUPLICATE KEY UPDATE product_qty=product_qty+".(int)$ar['qty'].", 
...
// 					$cartArr[$customer_id][$ar['product_price_id']]['qty'] = (int)$ar['product_qty'];	
...
// */
// function getCartData($cartArr='', $customer_id=0, $is_from_database=false, $is_ajax=false, $is_cart_or_checkout=false, $is_status_check=true)
// {
// 	$CI =& get_instance();
...
// 	$CI =& get_instance(); 
// 	$sql = 'SELECT s.pincode_id,service_type_code, cod_limit, prepaid_limit, shipping_method_key FROM ';
// 	if($pincode_id!=0)
// 	{
...
// 			WHERE s.shipping_pincodes_status=0 AND p.pincode='".$pincode."'
// 			GROUP BY m.shipping_method_key ";	
...
// 						$join .= "INNER JOIN diamond_price dpcs ON dpcs.diamond_price_id=pp.cs_diamond_price_id ";	
// 					}
// 					else
// 					{
...
// 						$join .= "INNER JOIN diamond_price dpss".$k." ON dpss".$k.".diamond_price_id=pp.ss".$k."_diamond_price_id ";	
// 					}
// 					else
// 					{
...
// 				$join .= "INNER JOIN metal_price mp 
...
// /**
//  * @author   Cloudwebs
//  * @abstract functoin will get grand total
//  */
// function cart_hlp_getGrandTotal( $is_front_end, $is_from_database, $is_validate_coupon=true )
// {
// 	$CI =& get_instance(); 
// 	$customer_id = cart_hlp_getCustOrdId( $is_front_end );
...
// 	{
// 		$resCouponArr = array();
// 		$resArr = getCartData( '', $customer_id, $is_from_database, false, true, true);
// 		if( $resArr['type'] == 'success' ) 
// 		{
...
// 							$order_details_is_returned, $order_details_return_quantity, $order_details_expected_delivery_date, $diamond_price_id='', $dp_price='', $product_type='', $warehouse_transactions_id=0 )
// {
...
// 		updateProductQuantity($product_id, $order_details_product_qty);	//formula: current_quantity - previous_quantity */
...
// 	return array( 'type'=>'success', 'msg'=>'Transaction inserted successfully.', 'order_transaction_id'=>$CI->db->insert_id()); 
// }	
...
// +-----------------------------------------------+
// 	@author Cloudwebs
// 	@abstract function will complete order after payment gateway process completes
...
// 		$data_email['es_message'] .= $CI->load->view( 'templates/footer-template', array( 'email_id' => $resArr['customer_emailid'] ), TRUE );  
...
// 		$tempMsg .= $CI->load->view( 'templates/footer-template', array('email_id'=>$data_email['es_to_emails']), TRUE);
...
// 		$data_email['es_message'] .= $CI->load->view( 'templates/footer-template', array('email_id'=>$data_email['es_to_emails']), TRUE );
...
// 				{
// 					$resArr['order_status_msg'] = 'Your order: '.$order_id.'  is shipped via <a href="'.$resOrd['shipping_method_url'].'">'.$resOrd['shipping_method_name'].'</a>. ';
...
// 				$data_email['es_message'] .= $CI->load->view('templates/footer-template', array('email_id'=>$resOrd['customer_emailid']), TRUE);
// 				$data_email['es_status'] = '';//$CI->input->post('order_status_id');
...
// 				sendMail( "hi0001234d@gmail.com", $data_email['es_subject'], $data_email['es_message']);	
// 				$CI->db->insert("email_send_history",$data_email);
...
// 					$msg = '';
// 					if($order_status_key == 'YET_TO_SHIP')
// 						$msg = 'Your Cloudwebs order '.$resArr['order_id'].' is shipped via '.$resOrd['shipping_method_name'].'. Your tracking id is '.$order_tracking_number.'. The tracking details will be activated within 24 hours.';
