...
function generateFilterHTMLTaskList( $k, $ar, $search=array() )
{
    ...
    if( $isContinue )
    {
        ?><li class="<?php echo ( $k == 0 ) ? 'active' : '';?>">
    		<a data-toggle="pill" href="#my-offer-<?php echo $ar->_id;?>" class="mob_opn-sidepanel nav-link" onclick="getJobDetails('<?php echo $ar->_id;?>');">
    			<div class="forhr">
    				<div class="row">
...
function generateHTMLBidsList( $jobListArr, $search=array() )
{
    ...
    {
        foreach ( $jobListArr->data as $k=>$ar )
            generateFilterHTMLBidsList( $k, $ar, $search );
    }
    else
        noResultFound();
 ...
 *
 */
function generateFilterHTMLBidsList( $k, $ar, $search=array() )
{
    $isContinue = false;
    if( !isEmptyArr( $search ) )//&& in_array( "sort_by" , $search ) !== false
    {
    ...
    { 
        ?>
        <div id="my-offer-<?php echo $ar->job_id;?>" class="tab-pane fade in <?php echo ( $k==0 ) ? 'active' : '';?>">
			<ul class="nav nav-tabs my-job_tab mobile_fix-tab">
				<li class=" nav-item active"><a class="nav-link" data-toggle="tab" href="#post-task-<?php echo $ar->job_id;?>">Task Info</a></li>
				<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#post-timeline-<?php echo $ar->job_id;?>">Timeline <i class="fa fa-circle unread-noti"></i></a></li>		  
				<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#post-offer-<?php echo $ar->job_id;?>">Chat <i class="fa fa-circle unread-noti"></i></a></li>		  
			</ul>
			<div class="tab-content">
				<div id="post-task-<?php echo $ar->job_id;?>" class="tab-pane fade position-relative active">
					<div class="job-status text-capitalize mt-4">
						<span class="badge <?php echo ( $ar->job_whole_data->job_status == "OPEN" ) ? 'active' : '';?>">Open</span>
						<span class="badge <?php echo ( $ar->job_whole_data->job_status == "ASSIGNED" ) ? 'active' : '';?>">Assigned</span>
						<span class="badge <?php echo ( $ar->job_whole_data->job_status == "COMPLETED" ) ? 'active' : '';?>">Completed</span>
						<span class="badge <?php echo ( $ar->job_whole_data->job_status == "EXPIRED" ) ? 'active' : '';?>">Expired</span>
					</div>
					<div class="clearfix">
						<h3 class="job-title mt-5 col-xs-9 col-sm-12"> <?php echo $ar->job_title;?> </h3>
						<div class="col-xs-3 pr-0 col-sm-12 position-static">
							<!--	price tag -->
							<div class="text-right w-100 clearfix">
								<div class="task-budget">
									<div class="price-tag">$<?php echo $ar->job_whole_data->total_budget;?></div>
								</div>	
							</div>
						</div>
					</div>
					<div class="mt-4">
						...
						<p class="text-muted detail-txt"><?php echo $ar->job_description;?> </p>												
					</div>
					<!--		attachment	 -->
					<?php 
					if( !isEmptyArr( $ar->job_whole_data->job_images ) )
					{
					   ?>  			
    					<div class="mt-5">
    						...
    						<div class="attach-imgs">
								<?php 
							    foreach ( $ar->job_whole_data->job_images as $var )
							    {
							        $ext = explode( "." , $var );
							        $ext = end( $ext );
							        ?>
				...
				<div id="post-offer-<?php echo $ar->job_id;?>" class="tab-pane fade in <?php echo ( $k==0 ) ? 'active' : '';?>">
					<div class="chat-section mt-5 px-3">
						<?php
    					$chatArr = array();
    					if( !empty( $ar->chat_group_details ) && isset( $ar->chat_group_details->_id ) )
    					{
    					    $chatArr = getTaskChatHistoryData( $ar->chat_group_details->_id."/" );
    					            ...
    					            <div class="chat-box clearfix">
    					            	...
    					            	if( $var->messege_belong == "OTHER" )
    					            	{
                							...
                							<div class="postByJob my-3 clearfix msg-box">
                								<div class="col-2">
                									<div class="user-img ">
                										<img class="img-responsive" src="images/job-dummy.png" />
                									</div>
                								</div>
                								<div class="mb-4 mt-0">
    					    ...
    					    <form onsubmit="return false;" id="chat_send_message">
    					    	<input type="text" name="group_id" value="<?php echo $ar->chat_group_details->_id?>" id="group_id">
    			...
    			<div id="post-timeline-<?php echo $ar->job_id;?>" class="tab-pane fade position-relative">
    				<?php 
    				$jobDetails = getJobDetails( $ar->job_id."/");
    				
    				if( isset( $jobDetails->status ) && $jobDetails->status == true )
    				{
    				    echo '<p class="text-center mt-5">You have '.$jobDetails->data->job_bids.' bids avilable for this job</p>';
    				    
    				    if( isset( $jobDetails->data->all_job_bids ) && !empty( $jobDetails->data->all_job_bids ) )
    				    {
    				        foreach ( $jobDetails->data->all_job_bids as $tl )
    				        {
    				            ?>
        				        <div class="offer-list offer-mob clearfix d-block">	
                					<div class="clearfix">
                    					...
                    					<div class="px-2 col-sm-3 col-xs-6 mt-3 text-right">
                    						<a class="btn btn-upload" data-toggle="modal" data-target="#offerAccept">Assigned</a>
                    					</div>
                					</div>	
                					<div class="notification clearfix text-center noti-msg mb-0">
                						 <?php echo $tl->bid_messege;?>					
                					</div>
                				</div>
                				<?php 
    				        }
    				    ...
    				    if( !isEmptyArr( $jobDetails->data->job_timeline  ) )
    				    {
    				        foreach ( $jobDetails->data->job_timeline as $tl )
    				        {
    				...
    				<div class="notification clearfix mt-80 ">
    					<div class="pull-left">
    				...
    				<div class="notification clearfix  ">
    					<div class="pull-left">
    						<i class="fa fa-info pr-2"></i> Payment pending
    					</div>	
    					<div class="pull-right"> 
    						 23 Nov, 2019
    					</div>
    				...
    				<div class="notification clearfix  ">
    					<div class="pull-left">
    						<i class="fa fa-info pr-2"></i> Payment released
    					</div>	
    					<div class="pull-right"> 
    						 23 Nov, 2019
    					</div>
 ...
 *
 */
function generateHTMLJobDetails( $jobListArr, $job_id, $myOfferArr=array(), $myBidsListArr=array() )
{
    if( $jobListArr->status == true )
    {
        $chatId = "";
        ...
        ?>
        <div id="my-offer-<?php echo $job_id;?>" class="tab-pane fade in active">
			<ul class="nav nav-tabs my-job_tab mobile_fix-tab">
				<li class="nav-item active"><a class="nav-link" data-toggle="tab" href="#post-task-<?php echo $job_id;?>">Task Info</a></li>
				<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#post-timeline-<?php echo $job_id;?>">Timeline <i class="fa fa-circle unread-noti"></i></a></li>		  
				<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#post-offer-<?php echo $job_id;?>">Chat <i class="fa fa-circle unread-noti"></i></a></li>		  
			</ul>
			<div class="tab-content">
				<div id="post-task-<?php echo $job_id;?>" class="tab-pane fade position-relative active in">
					<div class="job-status text-capitalize mt-4">
				...
				<div id="post-timeline-<?php echo $job_id;?>" class="tab-pane fade position-relative">
    				<?php 
    			...
    			<div id="post-offer-<?php echo $job_id;?>" class="tab-pane fade in">
					<div class="chat-section mt-5 px-3">
						<?php
    					$chatArr = array();
    					if( !empty( $chatId ) )
					...
					<div class="form-group px-3 mt-4">
						<textarea placeholder="Ask A Question" class="form-control" rows="3"></textarea>
					</div>
					<div class="clearfix">
						<div class="position-relative col-xs-6">
							<div class="upload-btn-wrapper ">
						...
						<div class="col-xs-6 text-right">
							<a href="#" class="btn btn-upload">Submit</a>
						</div>
					</div>
				</div>
    		</div>
    ...
    {
        ?>
        <div id="my-offer-0" class="tab-pane fade in">
    		<ul class="nav nav-tabs my-job_tab mobile_fix-tab">
