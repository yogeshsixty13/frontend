...
function postDataSubmit( $url, $data_string )
{
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, str_ireplace("+", " ", $data_string ) );
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
...
{
	$resultArr = array();
	if( isset($jobListArr->status ) && $jobListArr->status == "true" )
    {
        ...
        { 
        	if($isGoogleMap)
			    ...
			    generateFilterHTMLTaskList( $k, $ar, $search, $resultArr );
        }
	}
	else
	{
 ...
 * 
 */
function generateFilterHTMLTaskList( $k, $ar, $search=array(), &$resultArr=array() )
{
    $isContinue = false;
    $isFilterContinue = false;
    if( !isEmptyArr( $search ) )// && in_array( "sort_by" , $search ) !== false
    {
        ...
        if( isset( $search['job_budget_type'] ) && isset( $ar->job_whole_data->job_budget_type ) && ( $ar->job_whole_data->job_budget_type == $search['job_budget_type'] || $search['job_budget_type'] == "ALL" ) )
            $isContinue = true;
        
        if( isset( $search['sort_by'] ) && isset( $ar->bid_status ) && $ar->bid_status == strtoupper( $search['sort_by'] ) )
            $isFilterContinue = true;
        
        if( isset( $search['sort_by'] ) && $search['sort_by'] == "ALL" )
            $isContinue = true;
        
        if( isset( $search['search_city'] ) && isset( $ar->location_data->city_name ) && ( $ar->location_data->city_name == str_ireplace( "%2C+", ", ", $search['search_city'] ) ) )
            $isContinue = true;
    }
    else
        $isContinue = true;
...
function generateHTMLBidsList( $jobListArr, $search=array() )
{
    ...
    {
        foreach ( $jobListArr->data as $k=>$ar )
            generateFilterHTMLTaskList( $k, $ar->job_whole_data, $search, $resultArr );
    }
    else
        noResultFound();
 ...
 *
 */
function generateHTMLJobDetails( $jobListArr, $job_id, $myOfferArr=array(), $myBidsListArr=array(), $id, $myContractsListArr=array(), $active="Task Info" )
{
    $CI =& get_instance();
    ...
    if( $jobListArr->status == "true" )
    {
        ...
        <div id="task-id-<?php echo $toggle_id;?>" class="tab-pane fade in active">
			<ul class="nav nav-tabs my-job_tab mobile_fix-tab">
				<li class="nav-item <?php echo ( $active == "Task Info" ) ? 'active' : '';?>" onclick="updateTabPane( 'Task Info' )"><a class="nav-link" data-toggle="tab" href="#post-task-<?php echo $toggle_id;?>">Task Info</a></li>
				<li class="nav-item <?php echo ( $active == "Timeline" ) ? 'active' : '';?>" onclick="updateTabPane( 'Timeline' )"><a class="nav-link" data-toggle="tab" href="#post-timeline-<?php echo $toggle_id;?>">Timeline <i class="fa fa-circle unread-noti"></i></a></li>		  
				<li class="nav-item <?php echo ( $active == "Chat" ) ? 'active' : '';?>" onclick="updateTabPane( 'Chat' )"><a class="nav-link" data-toggle="tab" href="#post-offer-<?php echo $toggle_id;?>">Chat <i class="fa fa-circle unread-noti"></i></a></li>		  
			</ul>
			<div class="tab-content">
				<div id="post-task-<?php echo $toggle_id;?>" class="tab-pane fade position-relative <?php echo ( $active == "Task Info" ) ? 'active in' : '';?>">
				...
				<div id="post-timeline-<?php echo $toggle_id;?>" class="tab-pane fade position-relative <?php echo ( $active == "Timeline" ) ? 'active in' : '';?>">
    				<?php 
    				if( in_array( $job_id , $myOfferArr ) )
    				{
				                ...
				                <div class="offer-list offer-mob clearfix d-block">	
                					<div class="clearfix">
                    					<div class="col-sm-2 pr-2 col-xs-3">
                    						<div class="user-img">
                    							<img class="img-responsive" src="<?php echo load_image($tl->bid_by->profile_image);?>" />								
                    						</div>
                    					...
                    					else if( $tl->bid_status == "UNSELECTED" )
                    					{
                    					    ?>
                    					    <div class="px-2 col-sm-3 col-xs-6 mt-3 text-right">
                        						<a class="btn btn-upload">Un Selected</a>
                        					</div>
                    					    <?php 
                    					}
                    					else
    			...
    			<div id="post-offer-<?php echo $toggle_id;?>" class="tab-pane fade <?php echo ( $active == "Chat" ) ? 'active in' : '';?>">
...
//     					    if( $chatArr->total_messege != $chatArr->total_messege_since_start)
//     					    {
...
//     					    }
    					    ...
    					    if( $chatArr->status == "true" )
    					    {
    					        ...
    					        <?php 
    					        foreach ( array_reverse( $chatArr->data ) as $var )
    					        {
    					            ?>
					       ...
					       <form onsubmit="return false;" id="chat_send_message">
								<span id="chat_job_id" class="d-none"><?php echo $job_id;?></span>
    					    	<input type="hidden" name="group_id" value="<?php echo str_replace( "/", "", $chatId );?>" id="group_id">
        					    <div class="form-group px-3 mt-4">
            						<textarea placeholder="Ask A Question" name="message" id="message" class="message form-control" rows="3" required="required"></textarea>
            					</div>
            					<div class="clearfix">
            						...
            						<div class="col-md-12 mt-3">
    									<span class="error_msg error" id="make_an_chat_error"></span>
        								<span class="success_msg error" id="make_an_chat_success"></span>
        							</div>
            						<div class="col-xs-12 text-right">
            							<input type="submit" class="btn btn-upload pull-right" id="submit_chat" onclick="submitChat();" value="Submit">
            						</div>
            					</div>
        					</form>
    					<?php 
    					    ...
    					    echo '<div class="col-md-12 text-center">No Comment Data</div>';
    					?>
					</div>
				</div>
    		</div>
 ...
 */
function getTaskChatHistoryData( $group_id='', $p=0 )
{
    $page = "";
    if( !empty( $p ) )
        $page = "?page=2";
    
    return getRequestedDataFromURL( 'chat/view/'.$group_id.$page );
}
