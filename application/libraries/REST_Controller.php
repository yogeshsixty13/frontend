    ...
    */
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
        ...
        // Otherwise (if no data but 200 provided) or some data, carry on camping!
			 ...
			 * It will tells REST client through REST response that session is change so that, 
			 * they can update thier particular session key cookie and so on. 
			 */
        	if( $this->session->userdata("is_SID_c") == 1 )
        	{        	
        		$data["is_SID_c"] = 1; 
        		$data["session_id"] = session_id(); 
        		
        		$this->session->set_userdata( array( "is_SID_c" => 0 ) ); 
        	}

        	/**
        	 * Cloudwebs
        	 * added on 04-07-2015 
        	 * It will tells REST client through REST response that cart wish count had been changed.
        	 */
        	if( $this->session->userdata("is_CW") == 1 )
        	{
        		$data["_CW"] = getCartWishCount();
        	
        		$this->session->set_userdata( array( "is_CW" => 0 ) );
        	}
        	 
        	/**
        	 * Cloudwebs
        	 * added on 15-05-2015 for to REST APIs to redirect to other app pages as per REST Server actions.
        	 */
        	if( $this->session->userdata("_redirect") !== FALSE )
        	{
        		$data["_redirect"] = $this->session->userdata("_redirect");
        		$data["_rparam"] = $this->session->userdata("_rparam");
        	
        		$this->session->unset_userdata( array( "_redirect"=>"", "_rparam"=>"" ) );
        	}
        	
        	/**
        	 * Cloudwebs
        	 * added on 15-05-2015 to set flash message in REST APIs responses
        	 */
        	$nt_arr = array('success', 'error', 'warning');
        	foreach($nt_arr as $nt)
        	{
        		$flMessage = getFlashMessage($nt);
        		if($flMessage != '')
        		{
        			$data["flash"][$nt] = $flMessage; 
        			break;
        		}
        	}
